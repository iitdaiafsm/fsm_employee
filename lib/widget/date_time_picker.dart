...
class _DateTimePickerState extends FormFieldState<String> {
  ...
  void set12HourTimeValues(final TimeOfDay ptTimePicked) {
    ...
    final lsHour = DateFormatHelper.convertDateFromDate(ldTime, "hh");
    final lsMinute = DateFormatHelper.convertDateFromDate(ldTime, "mm");
    ...
    final ldDatePicked = await showDatePicker(
      ...
      builder: (BuildContext context, Widget? child) {
        return MediaQuery(
          ...
          child: Theme(data: Get.theme, child: child ?? const SizedBox()),
        );
      },
    );
